// Make sure the file extension is .cu and not .cpp
// to compile the program first run: nvcc cudamatrix.cu -o c
// now to run the program use: ./c
// for nvcc command not error:
//    run : sudo apt install cuda-toolkit
//    then check install with : nvcc --version
//    if no error continue compiling and running the code
#include <iostream>
